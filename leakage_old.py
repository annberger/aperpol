    ...
    for beam in range(40):
        if os.path.isfile(self.polanalysisleakagedirV + '/V_' + str(beam).zfill(2) + '_pos.fits'):
            try:
                ...
                cat_v = cat_v['RA', 'E_RA', 'DEC', 'E_DEC', 'Total_flux', 'E_Total_flux', 'Peak_flux', 'E_Peak_flux', 'Xposn', 'Yposn']
                cat_v.rename_column('Xposn', 'Pix_x')
                cat_v.rename_column('Yposn', 'Pix_y')
                ...
                print(str(beam).zfill(2) + ' not successfully processed!')
